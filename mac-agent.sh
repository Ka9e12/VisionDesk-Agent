...
print_help() {
  cat <<'EOF'
VisionDesk Agent macOS/Linux 一键脚本

用法：
  ./mac-agent.sh                         自动配置后进入交互模式
  ./mac-agent.sh "你的任务"               直接执行一个任务
  ./mac-agent.sh run "你的任务" [参数]     直接透传到 desktop-agent run
  ./mac-agent.sh doctor                  检查环境
  ./mac-agent.sh perceive                截图并输出当前感知
  ./mac-agent.sh setup                   只安装/修复环境，不执行任务
  ./mac-agent.sh cli [desktop-agent参数]  使用原始 CLI
  ./mac-agent.sh -help                   显示帮助
...
常用参数：
  ...
  --accessibility                    启用系统辅助功能探测，需要配合 run/perceive
...
例子：
  ./mac-agent.sh
  ./mac-agent.sh "打开 Safari 搜索 AI Agent，并总结重点"
  ./mac-agent.sh run "打开备忘录写一条测试笔记" --dry-run
EOF
}
